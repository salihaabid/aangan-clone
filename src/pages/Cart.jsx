...
//         {/* Cart heading */}
//         <h2 className='text-6xl font-medium text-[#2a4125]'>Your cart</h2>
//         <Link
//           to='/Collection'
//           className='text-[#2a4125] underline hover:text-[#3a5135] text-2xl'
//         >
//           Continue shopping
//         </Link>
//       </div>
//       {/* Header row */}
//       <div className='grid grid-cols-3 text-[#77846a] text-sm font-medium  py-5 pb-8'>
...
//             navigate('/CheckOut'); // ✅ navigate after clearing
//           }}
//         >
//           Check out
//         </button>
    ...
    <div className='max-w-6xl mx-auto p-6'>
      ...
      <div className='space-y-4'>
        {cartData.map((item) => (
          <div
            key={`${item.id}-${item.size}`}
            className='flex flex-wrap items-center justify-between gap-6 py-6 border-b border-gray-200'
          >
            {/* Product info */}
            <Link
              className='flex items-center space-x-4 flex-1 min-w-[200px]'
              to={`/product/${item.id}`}
            >
              <img
                src={item.image}
              ...
              <div className='flex flex-col'>
                <h3 className='text-2xl sm:text-xl font-medium text-[#2a4125] leading-snug max-w-[300px] break-words'>
                  {item.name}
                </h3>
            ...
            <div className='flex items-center space-x-1 sm:space-x-3'>
              <button
                className='px-3 py-1 border border-[#2a4125] rounded '
                onClick={() =>
                  updateQuantity(item.id, item.size, item.quantity - 1)
                }
                disabled={item.quantity <= 1}
              ...
              <button
                className='px-3 py-1 border rounded-full text-lg font-medium hover:bg-gray-100'
                onClick={() =>
                  updateQuantity(item.id, item.size, item.quantity + 1)
                }
              >
            ...
            <div className='flex items-center space-x-4 min-w-[120px] justify-end'>
              <span className='font-semibold text-[#2a4125] text-lg'>
                Rs.{item.price * item.quantity}.00
              </span>
              <button
